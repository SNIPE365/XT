...
set VM_NAME=hello_test
set FLOPPY_CTL=Floppy
set FLOPPY_IMG=%SETUP_DIR%\boot.img
...
:: Check if floppy controller exists
...
if errorlevel 1 (
    ...
    "%VBOX_PATH%\VBoxManage" storagectl "%VM_NAME%" --name "%FLOPPY_CTL%" --add floppy
) else (
    echo Floppy controller already exists, skipping add.
)

:: Check if floppy image is attached
"%VBOX_PATH%\VBoxManage" showvminfo "%VM_NAME%" --machinereadable | findstr /i /c:"%FLOPPY_CTL%-0-0" | findstr /i /c:"%FLOPPY_IMG%" >nul
if errorlevel 1 (
    echo Attaching floppy image...
    "%VBOX_PATH%\VBoxManage" storageattach "%VM_NAME%" --storagectl "%FLOPPY_CTL%" --port 0 --device 0 --type fdd --medium "%FLOPPY_IMG%"
) else (
    echo Floppy image already attached, skipping attach.
)
